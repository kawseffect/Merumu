    ...
    if (subCommand === 'add') {
      ...
      interaction.reply({
        embed: {
          ...
          description:
            client.emotes +
            ` ***<@${userId.id}> (${userId.id}) has been warned***`
        }
      });
      ...
      interaction
        .reply({
          embed: {
            ...
            description:
              client.emotes +
              `***<@${userId.id}> (${userId.id}) has been unwarned***`
          }
        })
        .catch(() => {
          interaction.reply({
      ...
      if (!isNaN(page)) {
        ...
        client.db.getWarnings(interaction.guild.id, userid).then((warnings) => {
          ...
          var array_chunks = Array(Math.ceil(warnings.length / 15))
            .fill()
            .map((_, index) => index * 15)
            .map((begin) => warnings.slice(begin, begin + 15));
          if (page > -1 && array_chunks.length > page) {
            interaction.reply({
              embed: {
                ...
                description:
                  client.emotes +
                  `  Warnings for <@${userid.id}>  (${
                    userid.id
                  })**\n\n Total warnings:  ${warnings.length} | Page: ${
                    page + 1
                  }/${array_chunks.length}\n\n${array_chunks[page]
                    .map(
                      (warning, index) =>
                        `${index + 1})‎ Timestamp: ${
                          ...
                          warning.issuer
                        }>\n *Reason for the warning: ${warning.reason}*`
                    )
                    .join('\n\n')}`
              }
            });
